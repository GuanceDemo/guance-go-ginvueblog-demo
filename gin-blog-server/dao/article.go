...
// 根据 id 获取文章详情
func (*Article) GetInfoById(id int, ctx *gin.Context) (res resp.FrontArticleDetailVO) {
	DB.WithContext(ctx.Request.Context()).Table("article").
		Preload("Category").
		Preload("Tags").
...
func (*Article) GetList(req req.GetArts, ctx *gin.Context) ([]resp.ArticleVO, int64) {
	var list = make([]resp.ArticleVO, 0)
	var total int64

	db := DB.WithContext(ctx.Request.Context()).Model(model.Article{})
...
func (*Article) GetFrontList(req req.GetFrontArts, ctx *gin.Context) ([]resp.FrontArticleVO, int64) {
	list := make([]resp.FrontArticleVO, 0)
	var total int64

	db := DB.WithContext(ctx.Request.Context()).Table("article").
...
// 查询最新的 n 篇文章
func (*Article) GetNewestList(n int, ctx *gin.Context) []resp.RecommendArticleVO {
...
// * 查询 上一篇文章 (id < 当前文章id)
func (*Article) GetLast(id int, ctx *gin.Context) (res resp.ArticlePaginationVO) {
	sub := DB.WithContext(ctx.Request.Context()).Table("article").
		Select("max(id)").
		Where("id < ?", id)
	DB.WithContext(ctx.Request.Context()).Table("article").
		Select("id, title, img").
...
// 查询 下一篇文章 (id > 当前文章id)
func (*Article) GetNext(id int, ctx *gin.Context) (res resp.ArticlePaginationVO) {
	DB.WithContext(ctx.Request.Context()).Table("article").
		Select("id, title, img").
...
// 查询 n 篇推荐文章 (根据标签)
func (*Article) GetRecommendList(id, n int, ctx *gin.Context) []resp.RecommendArticleVO {
	list := make([]resp.RecommendArticleVO, 0)

	// sub1: 查出标签id列表
	// SELECT tag_id FROM `article_tag` WHERE `article_id` = ?
	sub1 := DB.WithContext(ctx.Request.Context()).Table("article_tag").
		Select("tag_id").
		Where("article_id", id)
	...
	// WHERE `article_id` != ?
	sub2 := DB.WithContext(ctx.Request.Context()).Table("(?) t1", sub1).
		Select("DISTINCT article_id").
		Joins("JOIN article_tag t ON t.tag_id = t1.tag_id").
		Where("article_id != ?", id)
	// 根据 文章id列表 查出文章信息 (前 6 个)
	DB.WithContext(ctx.Request.Context()).Table("(?) t2", sub2).
		Select("id, title, img, created_at").
