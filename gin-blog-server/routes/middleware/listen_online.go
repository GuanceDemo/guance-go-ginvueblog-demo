...
func ListenOnline() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if s, err := utils.Redis.GetResult(userKey, c); err == nil {
			utils.Json.Unmarshal(s, &sessionInfo)
		} else if s, err := utils.Redis.GetResult(KEY_DELETE+uuid, c); err == nil { // 被强制离线
			utils.Json.Unmarshal(s, &sessionInfo)
		...
		if sessionInfo.IsOffline == 1 { // 被强制下线
			utils.Redis.Del(userKey, c) // 删除 redis 中缓存
		...
		utils.Redis.Set(KEY_USER+uuid, utils.Json.Marshal(sessionInfo), 10*time.Minute, c)
