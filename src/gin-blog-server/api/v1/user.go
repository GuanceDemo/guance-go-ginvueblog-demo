...
func (*User) UpdateCurrent(c *gin.Context) {
	...
	r.SendCode(c, userService.UpdateCurrent(currentUser, c))
}

// 更新当前用户信息: 关联处理 [用户-角色] 关系
func (*User) Update(c *gin.Context) {
	r.SendCode(c, userService.Update(utils.BindJson[req.UpdateUser](c), c))
}

func (*User) GetList(c *gin.Context) {
	r.SuccessData(c, userService.GetList(utils.BindQuery[req.GetUsers](c), c))
}

// 修改用户禁用状态
func (*User) UpdateDisable(c *gin.Context) {
	req := utils.BindValidJson[req.UpdateUserDisable](c)
	userService.UpdateDisable(req.ID, *req.IsDisable, c)
	r.Success(c)
}

// 修改普通用户密码: 管理员可以直接修改
func (*User) UpdatePassword(c *gin.Context) {
	r.SendCode(c, userService.UpdatePassword(utils.BindJson[req.UpdatePassword](c), c))
}
...
func (*User) UpdateCurrentPassword(c *gin.Context) {
	r.SendCode(c, userService.UpdateCurrentPassword(
		...
		utils.GetFromContext[int](c, "user_info_id"), c))
}

// 获取在线用户列表
func (*User) GetOnlineList(c *gin.Context) {
	r.SuccessData(c, userService.GetOnlineList(utils.BindPageQuery(c), c))
}
...
func (*User) ForceOffline(c *gin.Context) {
	...
	r.SendCode(c, userService.ForceOffline(utils.BindJson[req.ForceOfflineUser](c), c))
}
